...
if ( class_exists( 'WC_Admin_Report' ) ) {
	...
	class WC_Report_Sales_By_TReport extends WC_Admin_Report {
		 ...
		 * Constructor
		 */
		public function __construct( $tax, $total_subtotal = '_line_total' ) {
		...
		public function get_products_in_category( $category_id ) {
			$term_ids = get_term_children( $category_id, $this->tax );
			$term_ids[] = $category_id;
			$product_ids = get_objects_in_term( $term_ids, $this->tax );

			return array_unique( apply_filters( 'woocommerce_report_sales_get_products', $product_ids, $category_id ) );
		}
		...
		public function get_chart_legend() {
			...
			foreach ( $this->show_categories as $category ) {
				$category = get_term( $category, $this->tax );
				$total = 0;
